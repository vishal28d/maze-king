...
class FluidNavBar extends StatefulWidget {
  static double nominalHeight = (Platform.isIOS ? (80 + MediaQuery.of(Get.context!).padding.bottom / 2) : 82.0);
  ...
  const FluidNavBar({super.key, required this.icons, this.onChange, this.style, required this.selectedTabBgColor, this.animationFactor = 1.0, this.scaleFactor = 1.2, this.defaultIndex = 0, FluidNavBarItemBuilder? itemBuilder})
...
class _FluidNavBarState extends State<FluidNavBar> with TickerProviderStateMixin {
  ...
  Widget build(context) {
    ...
    var height = FluidNavBar.nominalHeight;

    return Container(
      // width: appSize.width,
      height: FluidNavBar.nominalHeight,
      color: widget.selectedTabBgColor,
      child: Stack(
        children: [
          Positioned(
            left: 0,
          ...
          Positioned(
            ...
            height: height,
            child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, mainAxisSize: MainAxisSize.min, children: _buildButtons()),
          ),
        ],
      ),
    );
    ...
    return widget.icons
        ...
        .map(
          (entry) => widget.itemBuilder(
            ...
            FluidNavBarItem(
              entry.value.title,
              entry.value.svgSelectedPath ?? entry.value.svgSelectedPath,
              entry.value.svgUnSelectedPath ?? entry.value.svgUnSelectedPath,
              entry.value.icon,
  ...
  void _handleTap(int index) {
    ...
    _yController.value = 1.0;
    _xController.animateTo(_indexToPosition(index) / MediaQuery.of(context).size.width, duration: const Duration(milliseconds: 620) * widget.animationFactor);
    Future.delayed(
...
class _BackgroundCurvePainter extends CustomPainter {
  ...
  void paint(canvas, size) {
    ...
    final anchorControlOffset = Tween<double>(begin: radius * _horizontalControlTop, end: radius * _horizontalControlBottom).transform(const LinearPointCurve(0.5, 0.75).transform(norm));
    // Point that slides up and down depending on distance for the target x position
    final dipControlOffset = Tween<double>(begin: radius * _pointControlTop, end: radius * _pointControlBottom).transform(const LinearPointCurve(0.5, 0.8).transform(norm));
    final y = Tween<double>(begin: _topY, end: _bottomY).transform(const LinearPointCurve(0.2, 0.7).transform(norm));
    final dist = Tween<double>(begin: _topDistance, end: _bottomDistance).transform(const LinearPointCurve(0.5, 0.0).transform(norm));
    final x0 = _x - dist / 2;
    final x1 = _x + dist / 2;
